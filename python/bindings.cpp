...
Tensor<T> numpy_to_tensor(py::array_t<T> arr) {
  ...
  bool is_contiguous =
      (buf.format == py::format_descriptor<T>::format() && buf.size > 0 &&
...
template <typename T>
py::array_t<T> tensor_to_numpy_zero_copy(const Tensor<T>& tensor,
                                         py::object owner) {
  ...
  py::capsule capsule(holder.get(),
                      [](void* p) { delete static_cast<OwnerHolder*>(p); });
...
// Legacy function for backward compatibility (creates a copy to manage
// lifetime)
template <typename T>
py::array_t<T> tensor_to_numpy(const Tensor<T>& tensor) {
  ...
  // Get data pointer from the held tensor (not the original, to ensure it stays
  // valid)
  ...
  py::capsule owner(new std::shared_ptr<Tensor<T>>(tensor_holder), [](void* p) {
  ...
  return py::array_t<T>(py_shape, py_strides,
...
void bind_tensor_operations(py::module& m, const std::string& name_suffix) {
  ...
  py::class_<Tensor<T>>(m, class_name.c_str())
      ...
      .def(py::init([](py::array_t<T> arr) { return numpy_to_tensor(arr); }),
      ...
      // Element access
      .def("__getitem__",
           [](const Tensor<T>& t, size_t index) { return t[index]; })
      .def("__setitem__",
           [](Tensor<T>& t, size_t index, T value) { t[index] = value; })
      .def("at", py::overload_cast<size_t>(&Tensor<T>::at, py::const_))
      .def("at", py::overload_cast<const std::vector<size_t>&>(&Tensor<T>::at,
                                                               py::const_))
      ...
      .def("__add__",
           [](const Tensor<T>& a, const Tensor<T>& b) { return a + b; })
      .def("__add__", [](const Tensor<T>& a, T scalar) { return a + scalar; })
      .def("__radd__", [](const Tensor<T>& a, T scalar) { return a + scalar; })
      .def("__sub__",
           [](const Tensor<T>& a, const Tensor<T>& b) { return a - b; })
      .def("__sub__", [](const Tensor<T>& a, T scalar) { return a - scalar; })
      .def("__rsub__",
           [](const Tensor<T>& a, T scalar) {
             Tensor<T> result = a;
      ...
      .def("__mul__",
           [](const Tensor<T>& a, const Tensor<T>& b) { return a * b; })
      .def("__mul__", [](const Tensor<T>& a, T scalar) { return a * scalar; })
      .def("__rmul__", [](const Tensor<T>& a, T scalar) { return a * scalar; })
      .def("__truediv__",
           [](const Tensor<T>& a, const Tensor<T>& b) { return a / b; })
      .def("__truediv__",
           [](const Tensor<T>& a, T scalar) { return a / scalar; })
      .def("__rtruediv__",
           [](const Tensor<T>& a, T scalar) {
             Tensor<T> result = a;
             for (size_t i = 0; i < result.size(); ++i) {
      ...
      .def(
          "__iadd__",
          [](Tensor<T>& a, const Tensor<T>& b) -> Tensor<T>& {
            a += b;
            return a;
          },
      ...
      .def("sum",
           static_cast<Tensor<T> (Tensor<T>::*)(size_t) const>(&Tensor<T>::sum))
      .def("mean", static_cast<T (Tensor<T>::*)() const>(&Tensor<T>::mean))
      .def("mean", static_cast<Tensor<T> (Tensor<T>::*)(size_t) const>(
                       &Tensor<T>::mean))
      .def("max", static_cast<T (Tensor<T>::*)() const>(&Tensor<T>::max))
      .def("max",
           static_cast<Tensor<T> (Tensor<T>::*)(size_t) const>(&Tensor<T>::max))
      .def("min", static_cast<T (Tensor<T>::*)() const>(&Tensor<T>::min))
      .def("min",
           static_cast<Tensor<T> (Tensor<T>::*)(size_t) const>(&Tensor<T>::min))
      .def("std", static_cast<T (Tensor<T>::*)() const>(&Tensor<T>::std))
      .def("std",
           static_cast<Tensor<T> (Tensor<T>::*)(size_t) const>(&Tensor<T>::std))
      .def("var", static_cast<T (Tensor<T>::*)() const>(&Tensor<T>::var))
      .def("var",
           static_cast<Tensor<T> (Tensor<T>::*)(size_t) const>(&Tensor<T>::var))
...
PYBIND11_MODULE(dragon_tensor, m) {
  ...
  m.def(
      "from_numpy_float",
      [](py::array_t<float> arr) { return numpy_to_tensor(arr); },
      "Create Tensor from numpy float array");

  m.def(
      "from_numpy_double",
      [](py::array_t<double> arr) { return numpy_to_tensor(arr); },
      "Create Tensor from numpy double array");

  m.def(
      "from_numpy_int",
      [](py::array_t<int32_t> arr) { return numpy_to_tensor(arr); },
      "Create Tensor from numpy int32 array");

  m.def(
      "from_numpy_long",
      [](py::array_t<int64_t> arr) { return numpy_to_tensor(arr); },
      "Create Tensor from numpy int64 array");

  // Helper function to convert pandas Series/DataFrame
  m.def(
      "from_pandas_series",
      [](py::object series) -> py::object {
        try {
          ...
          if (dtype.is(py::dtype::of<float>())) {
            return py::cast(
                numpy_to_tensor(py::cast<py::array_t<float>>(values)));
          } else if (dtype.is(py::dtype::of<double>())) {
            return py::cast(
                numpy_to_tensor(py::cast<py::array_t<double>>(values)));
          } else if (dtype.is(py::dtype::of<int32_t>())) {
            return py::cast(
                numpy_to_tensor(py::cast<py::array_t<int32_t>>(values)));
          } else if (dtype.is(py::dtype::of<int64_t>())) {
            return py::cast(
                numpy_to_tensor(py::cast<py::array_t<int64_t>>(values)));
          } else {
            throw std::runtime_error("Unsupported pandas dtype");
          }
        } catch (const std::exception& e) {
          throw std::runtime_error("Failed to convert pandas Series: " +
                                   std::string(e.what()));
        }
      },
  ...
  m.def(
      ...
      [](py::object df) -> py::object {
        try {
          ...
          throw std::runtime_error("Failed to convert pandas DataFrame: " +
                                   std::string(e.what()));
        }
      },
  ...
  m.def(
      "from_torch",
      [](py::object torch_tensor) -> py::object {
        try {
          ...
          // Now convert numpy to tensor (will be efficient for contiguous
          // arrays)
          if (dtype.is(py::dtype::of<float>())) {
            return py::cast(numpy_to_tensor(py::cast<py::array_t<float>>(arr)));
          } else if (dtype.is(py::dtype::of<double>())) {
            return py::cast(
                numpy_to_tensor(py::cast<py::array_t<double>>(arr)));
          } else if (dtype.is(py::dtype::of<int32_t>())) {
            return py::cast(
                numpy_to_tensor(py::cast<py::array_t<int32_t>>(arr)));
          } else if (dtype.is(py::dtype::of<int64_t>())) {
            return py::cast(
                numpy_to_tensor(py::cast<py::array_t<int64_t>>(arr)));
          } else {
            throw std::runtime_error("Unsupported torch dtype");
          }
        } catch (const std::exception& e) {
          throw std::runtime_error("Failed to convert torch tensor: " +
                                   std::string(e.what()));
        }
      },
      "Create Tensor from torch tensor (zero-copy when torch tensor is CPU and "
      "contiguous)");
}
