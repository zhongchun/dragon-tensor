...
template<typename T>
Tensor<T>::Tensor(const std::vector<size_t>& shape, T fill_value) : shape_(shape) {
    validate_shape(shape);
    size_t total_size = std::accumulate(shape.begin(), shape.end(), size_t(1), 
                                        std::multiplies<size_t>());
...
template<typename T>
Tensor<T>::Tensor(const Tensor& other) : shape_(other.shape_), data_(other.data_) {}
...
Tensor<T> Tensor<T>::returns() const {
    if (ndim() != 1 || data_.size() < 2) {
        throw std::runtime_error("Returns calculation requires 1D tensor with at least 2 elements");
    }
...
Tensor<T> Tensor<T>::rolling_mean(size_t window) const {
    if (ndim() != 1) {
        throw std::runtime_error("Rolling operations only supported for 1D tensors");
    }
...
Tensor<T> Tensor<T>::correlation(const Tensor& other) const {
    if (!shapes_match(other)) {
        throw std::runtime_error("Tensors must have matching shapes for correlation");
    }
    if (data_.size() < 2) {
...
Tensor<T> Tensor<T>::covariance(const Tensor& other) const {
    if (!shapes_match(other)) {
        throw std::runtime_error("Tensors must have matching shapes for covariance");
    }
    if (data_.size() < 2) {
